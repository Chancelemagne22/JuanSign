...
export default function VerifyEmailPrompt({
  ...
  return (
    <div className="w-full max-w-md rounded-3xl border-4 border-[#BF7B45] bg-[#F7B27D] px-6 py-8 text-center shadow-[0_10px_25px_rgba(0,0,0,0.2)]">
      <h1 className="text-[#5D3A1A] font-black text-2xl">Verify Your Email</h1>
      ...
      <div className="mt-4 rounded-xl bg-[#FAD3AE] border border-[#D4956A] px-4 py-3">
        <p className="text-[#7B3F00] text-xs font-bold uppercase tracking-wide">Registered Email</p>
        <p className="text-[#5D3A1A] text-sm font-black break-all mt-1">{email || 'No email found'}</p>
      </div>
      ...
      >
        {isResendDisabled && resendSecondsLeft > 0 ? `Resend in ${resendSecondsLeft}s` : 'Resend Email'}
      </button>
