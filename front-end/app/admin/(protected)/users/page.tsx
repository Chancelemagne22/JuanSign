...
function Checkbox({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      ...
      style={{
        borderColor: checked ? '#B5621E' : '#C17A3A',
        backgroundColor: checked ? '#B5621E' : 'transparent',
      }}
...
function SortIcon({ active, asc }: { active: boolean; asc: boolean }) {
  return (
    <span className="ml-1 inline-block" style={{ color: active ? GOLD : '#C9A97A', fontSize: '0.65rem' }}>
      {active ? (asc ? '▲' : '▼') : '⇅'}
    </span>
  )
}
        ...
        <div className="flex flex-col items-center justify-center px-10 py-6 border-l" style={{ borderColor: DIVIDER }}>
          ...
          <div className="flex items-center gap-1 mt-1">
            <span
              ...
              style={{ backgroundColor: user.status === 'Active' ? '#22C55E' : '#9CA3AF' }}
            />
            <span
              style={{
                fontFamily: FONT,
                ...
                return (
                  <div
                    ...
                    style={{
                      gridTemplateColumns: '40px 90px 1fr 1fr 120px 100px 90px',
                      borderColor: DIVIDER,
                      backgroundColor: isRowSelected ? '#FFF0DC' : 'transparent',
                    }}
                    onMouseEnter={(e) => {
                      if (!isRowSelected)
                        (e.currentTarget as HTMLDivElement).style.backgroundColor = '#FFF5E6'
                    }}
                    onMouseLeave={(e) => {
                      if (!isRowSelected)
            ...
            {totalPages > 1 && (
              ...
              >
                <button
                  ...
                  style={{ fontFamily: FONT, color: BROWN, backgroundColor: '#F4E0B0' }}
                >
                  ‹ Prev
                </button>
                <span style={{ fontFamily: FONT, color: BROWN, fontSize: '0.97rem' }}>
