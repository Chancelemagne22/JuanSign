...
const inputStyle: React.CSSProperties = {
  ...
  backgroundColor: '#FFF', border: `1.5px solid ${INPUT_BORDER}`,
...
function BtnPrimary({ children, onClick, disabled }: { children: React.ReactNode; onClick?: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      disabled={disabled}
      style={{
        fontFamily: FONT, color: '#FFF', fontSize: '0.95rem', fontWeight: 700,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mt-auto">
        {!isNew && onDelete
          ? <button onClick={onDelete} style={{ fontFamily: FONT, color: '#B91C1C', fontSize: '0.88rem', background: 'none', border: 'none', cursor: 'pointer' }}>Delete lesson</button>
          : <span />}
...
function QuestionForm({
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          {(['identify', 'perform'] as QuestionType[]).map(t => (
            <button
              ...
              style={{
                ...
                backgroundColor: form.question_type === t ? '#EAF3E8' : '#FFF',
      ...
      <div className="flex items-center justify-between mt-auto">
        {!isNew && onDelete
          ? <button onClick={onDelete} style={{ fontFamily: FONT, color: '#B91C1C', fontSize: '0.88rem', background: 'none', border: 'none', cursor: 'pointer' }}>Delete question</button>
          : <span />}
        <div className="flex gap-3">
          <BtnSecondary onClick={onCancel}>Cancel</BtnSecondary>
    ...
    >
      <div className="rounded-2xl p-6 w-full max-w-sm" style={{ backgroundColor: CREAM }}>
        ...
        <Field label="Level Name">
          <input
            type="text" value={name} placeholder="e.g. Level 1 — Alphabet Basics"
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Order / Sequence">
            <input
              type="number" value={order} min={1} style={inputStyle}
              onChange={e => setOrder(parseInt(e.target.value) || 1)}
            />
          </Field>
          <Field label="Passing Score (%)">
            <input
              type="number" value={passingScore} min={1} max={100} style={inputStyle}
...
export default function AdminLevelsPage() {
  ...
  const handleSaveQuestion = async (form: Omit<Question, 'question_id'> & { question_id?: string }) => {
    ...
    if (form.question_id) {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Failed to update')
      setQuestions(prev => prev.map(q => q.question_id === form.question_id ? { ...q, ...form } as Question : q))
    ...
    <div className="flex flex-col gap-5 h-full">
      ...
      {toast && (
        <div
          ...
          style={{ fontFamily: FONT, color: '#fff', backgroundColor: toast.ok ? GREEN : '#B91C1C', fontSize: '0.97rem' }}
        >
          {toast.msg}
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-5 flex-1">
          ...
          <div className="rounded-2xl p-5 flex flex-col" style={{ backgroundColor: CREAM, minHeight: '400px' }}>
            ...
            <button
              ...
              style={{ fontFamily: FONT, color: '#FFF', backgroundColor: GREEN, border: 'none', cursor: 'pointer', fontSize: '0.95rem' }}
            >
